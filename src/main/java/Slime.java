...
public class Slime {
    ...
    private final Trail[][] previousTrails;
    ...
    private void createTrailMap(String species, Color color, int mapWidth, int mapHeight) {
        for (int i = 0; i < mapWidth; i++) {
            for (int j = 0; j < mapHeight; j++) {
                ...
                previousTrails[i][j] = new Trail(species, Trail.MIN_INTENSITY, color);
            }
        }
    }
    ...
    private double findMaxIntensityForAngleRange(double currentAngle, SlimeCell cell) {
        ...
        for (double angle = currentAngle - turnAngleWidth; angle < currentAngle + turnAngleWidth; angle += turnAngleStep) {
            ...
            for (int i = 0; i < configuration.SIGHT; i++) {
                if (cell.getPosX() > configuration.SIGHT
                        && cell.getPosX() < configuration.B_WIDTH - configuration.SIGHT
                        && cell.getPosY() > configuration.SIGHT
                        && cell.getPosY() < configuration.B_HEIGHT - configuration.SIGHT) {
    ...
    public void evaporateTrails() {
        for (int i = 0; i < trailMap.length; i++) {
            for (int j = 0; j < trailMap[0].length; j++) {
                previousTrails[i][j] = new Trail(trailMap[i][j].getSpecies(), trailMap[i][j].getIntensity(), trailMap[i][j].getColor());
            }
        }
